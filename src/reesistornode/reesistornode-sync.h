...
static const int SMARTNODE_SYNC_RESET_SECONDS = 600; // Reset fReachedBestHeader in CReesistornodeSync::Reset if UpdateBlockTip hasn't been called for this seconds

extern CReesistornodeSync reesistornodeSync;

//
// CReesistornodeSync : Sync reesistornode assets in stages
//
...
private:
    ...
    // Time when current reesistornode asset sync started
    std::atomic<int64_t> nTimeAssetSyncStarted{0};
    // ... last bumped
    std::atomic<int64_t> nTimeLastBumped{0};

    /// Set to true if best header is reached in CReesistornodeSync::UpdatedBlockTip
