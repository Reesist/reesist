...
CReesistornodePayments mnpayments;
...
*
*   Why is this needed?
*   - In Reesist some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    if(!reesistornodeSync.IsSynced() || fDisableGovernance) {
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward, CAmount specialTxFees)
{
    ...
    // we are still using budgets, but we have no data about them anymore,
    // we can only check reesistornode payments

    const Consensus::Params& consensusParams = Params().GetConsensus();

    if(nBlockHeight < consensusParams.reesistornodePaymentFixedBlock && nBlockHeight < consensusParams.nSuperblockStartBlock) {
    ...
    // Check for correct reesistornode payment
    if(CReesistornodePayments::IsTransactionValid(txNew, nBlockHeight, blockReward, specialTxFees)) {
        LogPrint(BCLog::MNPAYMENTS, "%s -- Valid reesistornode payment at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
        return true;
    }

    LogPrintf("%s -- ERROR: Invalid reesistornode payment detected at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
    return false;
}

void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& voutReesistornodePaymentsRet, std::vector<CTxOut>& voutSuperblockPaymentsRet, CAmount specialTxFees)
{
    ...
    if (!CReesistornodePayments::GetReesistornodeTxOuts(nBlockHeight, blockReward, voutReesistornodePaymentsRet, specialTxFees)) {
        LogPrint(BCLog::MNPAYMENTS, "%s -- no reesistornode to pay (MN list probably empty)\n", __func__);
    }

    txNew.vout.insert(txNew.vout.end(), voutReesistornodePaymentsRet.begin(), voutReesistornodePaymentsRet.end());
    txNew.vout.insert(txNew.vout.end(), voutSuperblockPaymentsRet.begin(), voutSuperblockPaymentsRet.end());

    std::string voutReesistornodeStr;
    for (const auto& txout : voutReesistornodePaymentsRet) {
        // subtract MN payment from miner reward
        txNew.vout[0].nValue -= txout.nValue;
        if (!voutReesistornodeStr.empty())
            voutReesistornodeStr += ",";
        voutReesistornodeStr += txout.ToString();
    }

    LogPrint(BCLog::MNPAYMENTS, "%s -- nBlockHeight %d blockReward %lld voutReesistornodePaymentsRet \"%s\" txNew %s", __func__, /* Continued */
                            nBlockHeight, blockReward, voutReesistornodeStr, txNew.ToString());
}

/**
*   GetReesistornodeTxOuts
*
*   Get reesistornode payment tx outputs
*/

bool CReesistornodePayments::GetReesistornodeTxOuts(int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& voutReesistornodePaymentsRet, CAmount specialTxFee)
{
    // make sure it's not filled yet
    voutReesistornodePaymentsRet.clear();

    if(!GetBlockTxOuts(nBlockHeight, blockReward, voutReesistornodePaymentsRet, specialTxFee)) {
        LogPrintf("CReesistornodePayments::%s -- no payee (deterministic reesistornode list empty)\n", __func__);
        return false;
    }
    ...
    for (const auto& txout : voutReesistornodePaymentsRet) {
        ...
        LogPrintf("CReesistornodePayments::%s -- Reesistornode payment %lld to %s\n", __func__, txout.nValue, EncodeDestination(dest));
    }

    return true;
}

bool CReesistornodePayments::GetBlockTxOuts(int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& voutReesistornodePaymentsRet, CAmount specialTxFee)
{
    voutReesistornodePaymentsRet.clear();

    CAmount reesistornodeReward = GetReesistornodePayment(nBlockHeight, blockReward, specialTxFee);
    ...
    if (dmnPayee->nOperatorReward != 0 && dmnPayee->pdmnState->scriptOperatorPayout != CScript()) {
        ...
        operatorReward = (reesistornodeReward * dmnPayee->nOperatorReward) / 10000;
        reesistornodeReward -= operatorReward;
    }

    if (reesistornodeReward > 0) {
        voutReesistornodePaymentsRet.emplace_back(reesistornodeReward, dmnPayee->pdmnState->scriptPayout);
    }
    if (operatorReward > 0) {
        voutReesistornodePaymentsRet.emplace_back(operatorReward, dmnPayee->pdmnState->scriptOperatorPayout);
    }

    return true;
}

bool CReesistornodePayments::IsTransactionValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward, CAmount specialTxFee)
{
    ...
    std::vector<CTxOut> voutReesistornodePayments;
    if (!GetBlockTxOuts(nBlockHeight, blockReward, voutReesistornodePayments, specialTxFee)) {
        LogPrintf("CReesistornodePayments::%s -- ERROR failed to get payees for block at height %s\n", __func__, nBlockHeight);
        return true;
    }
    ...
    for (const auto& txout : voutReesistornodePayments) {
        ...
        if (!found) {
            ...
            if (!ExtractDestination(txout.scriptPubKey, dest))
                assert(false);
            LogPrintf("CReesistornodePayments::%s -- ERROR failed to find expected payee %s in block at height %s\n", __func__, EncodeDestination(dest), nBlockHeight);
            return false;
        }
    }
    return true;
