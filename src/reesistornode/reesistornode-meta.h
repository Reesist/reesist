...
// Holds extra (non-deterministic) information about reesistornodes
// This is mostly local information, e.g. about mixing and governance
class CReesistornodeMetaInfo
{
    friend class CReesistornodeMetaMan;
...
public:
    CReesistornodeMetaInfo() = default;
    explicit CReesistornodeMetaInfo(const uint256& _proTxHash) : proTxHash(_proTxHash) {}
    CReesistornodeMetaInfo(const CReesistornodeMetaInfo& ref) :
...
typedef std::shared_ptr<CReesistornodeMetaInfo> CReesistornodeMetaInfoPtr;

class CReesistornodeMetaMan
{
private:
    ...
    std::map<uint256, CReesistornodeMetaInfoPtr> metaInfos;
    std::vector<uint256> vecDirtyGovernanceObjectHashes;

    // keep track of dsq count to prevent reesistornodes from gaming coinjoin queue
    int64_t nDsqCount = 0;
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        std::vector<CReesistornodeMetaInfo> tmpMetaInfo;
        if (ser_action.ForRead()) {
            READWRITE(tmpMetaInfo);
            metaInfos.clear();
            for (auto& mm : tmpMetaInfo) {
                metaInfos.emplace(mm.GetProTxHash(), std::make_shared<CReesistornodeMetaInfo>(std::move(mm)));
            }
        } else {
...
public:
    CReesistornodeMetaInfoPtr GetMetaInfo(const uint256& proTxHash, bool fCreate = true);
...
extern CReesistornodeMetaMan mmetaman;
