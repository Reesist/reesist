...
class CReesistornodeSync;
CReesistornodeSync reesistornodeSync;

CReesistornodeSync::CReesistornodeSync()
{
    nTimeAssetSyncStarted = GetTime();
    nTimeLastBumped = GetTime();
}

void CReesistornodeSync::Reset(bool fForce, bool fNotifyReset)
{
...
void CReesistornodeSync::BumpAssetLastTime(const std::string& strFuncName)
{
    if (IsSynced()) return;
    nTimeLastBumped = GetTime();
    LogPrint(BCLog::MNSYNC, "CReesistornodeSync::BumpAssetLastTime -- %s\n", strFuncName);
}

std::string CReesistornodeSync::GetAssetName() const
{
...
void CReesistornodeSync::SwitchToNextAsset(CConnman& connman)
{
    switch(nCurrentAsset)
    {
        case(SMARTNODE_SYNC_BLOCKCHAIN):
            LogPrintf("CReesistornodeSync::SwitchToNextAsset -- Completed %s in %llds\n", GetAssetName(), GetTime() - nTimeAssetSyncStarted);
            nCurrentAsset = SMARTNODE_SYNC_GOVERNANCE;
            LogPrintf("CReesistornodeSync::SwitchToNextAsset -- Starting %s\n", GetAssetName());
            break;
        case(SMARTNODE_SYNC_GOVERNANCE):
            ...
            LogPrintf("CReesistornodeSync::SwitchToNextAsset -- Sync has finished\n");

            break;
    }
    nTriedPeerCount = 0;
    nTimeAssetSyncStarted = GetTime();
    BumpAssetLastTime("CReesistornodeSync::SwitchToNextAsset");
}

std::string CReesistornodeSync::GetSyncStatus() const
{
    switch (nCurrentAsset) {
...
void CReesistornodeSync::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv) const
{
...
void CReesistornodeSync::ProcessTick(CConnman& connman)
{
    static int nTick = 0;
    nTick++;
    ...
    if(GetTime() - nTimeLastProcess > 60*60 && !fReesistornodeMode) {
        LogPrintf("CReesistornodeSync::ProcessTick -- WARNING: no actions for too long, restarting sync...\n");
        Reset(true);
    ...
    LogPrintf("CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTriedPeerCount %d nSyncProgress %f\n", nTick, nCurrentAsset, nTriedPeerCount, nSyncProgress);
    ...
    for (auto& pnode : vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            if ((pnode->fWhitelisted || pnode->m_manual_connection) && !netfulfilledman.HasFulfilledRequest(pnode->addr, strAllow)) {
                ...
                LogPrintf("CReesistornodeSync::ProcessTick -- skipping mnsync restrictions for peer=%d\n", pnode->GetId());
            }
            ...
            if(netfulfilledman.HasFulfilledRequest(pnode->addr, "full-sync")) {
                ...
                LogPrintf("CReesistornodeSync::ProcessTick -- disconnecting from recently synced peer=%d\n", pnode->GetId());
                continue;
            }
            ...
            if(!netfulfilledman.HasFulfilledRequest(pnode->addr, "spork-sync")) {
                ...
                LogPrintf("CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- requesting sporks from peer=%d\n", nTick, nCurrentAsset, pnode->GetId());
            }
                    ...
                    if (gArgs.GetBoolArg("-syncmempool", DEFAULT_SYNC_MEMPOOL)) {
                        ...
                        for (auto pNodeTmp : vNodesCopy) {
                            ...
                            if (pNodeTmp->nVersion >= 70216 && !pNodeTmp->fInbound && !fRequestedEarlier) {
                                ...
                                LogPrintf("CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- syncing mempool from peer=%d\n", nTick, nCurrentAsset, pNodeTmp->GetId());
                            }
                        }
                    }
                }
            ...
            if(nCurrentAsset == SMARTNODE_SYNC_GOVERNANCE) {
                ...
                LogPrint(BCLog::GOBJECT, "CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick, nCurrentAsset, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);

                // check for timeout first
                if(GetTime() - nTimeLastBumped > SMARTNODE_SYNC_TIMEOUT_SECONDS) {
                    LogPrintf("CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- timeout\n", nTick, nCurrentAsset);
                    if(nTriedPeerCount == 0) {
                        LogPrintf("CReesistornodeSync::ProcessTick -- WARNING: failed to sync %s\n", GetAssetName());
    ...
    for (auto& pnode : vNodesCopy) {
        ...
        if(nObjsLeftToAsk == 0) {
            ...
            if(GetTime() - nTimeNoObjectsLeft > SMARTNODE_SYNC_TIMEOUT_SECONDS && governance.GetVoteCount() - nLastVotes < std::max(int(0.0001 * nLastVotes), SMARTNODE_SYNC_TICK_SECONDS)) {
                LogPrintf("CReesistornodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- asked for all objects, nothing to do\n", nTick, SMARTNODE_SYNC_GOVERNANCE);
...
void CReesistornodeSync::SendGovernanceSyncRequest(CNode* pnode, CConnman& connman)
{
    CNetMsgMaker msgMaker(pnode->GetSendVersion());
...
void CReesistornodeSync::AcceptedBlockHeader(const CBlockIndex *pindexNew)
{
    LogPrint(BCLog::MNSYNC, "CReesistornodeSync::AcceptedBlockHeader -- pindexNew->nHeight: %d\n", pindexNew->nHeight);

    if (!IsBlockchainSynced()) {
        // Postpone timeout each time new block header arrives while we are still syncing blockchain
        BumpAssetLastTime("CReesistornodeSync::AcceptedBlockHeader");
    }
}

void CReesistornodeSync::NotifyHeaderTip(const CBlockIndex *pindexNew, bool fInitialDownload, CConnman& connman)
{
    if (pindexNew == nullptr) {
        return;
    }
    LogPrint(BCLog::MNSYNC, "CReesistornodeSync::NotifyHeaderTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
    if (IsSynced())
        return;

    if (!IsBlockchainSynced()) {
        // Postpone timeout each time new block arrives while we are still syncing blockchain
        BumpAssetLastTime("CReesistornodeSync::NotifyHeaderTip");
    }
}

void CReesistornodeSync::UpdatedBlockTip(const CBlockIndex *pindexNew, bool fInitialDownload, CConnman& connman)
{
    LogPrint(BCLog::MNSYNC, "CReesistornodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
    ...
    if (!IsBlockchainSynced()) {
        // Postpone timeout each time new block arrives while we are still syncing blockchain
        BumpAssetLastTime("CReesistornodeSync::UpdatedBlockTip");
    }
    ...
    LogPrint(BCLog::MNSYNC, "CReesistornodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d pindexTip->nHeight: %d fInitialDownload=%d fReachedBestHeader=%d\n",
                pindexNew->nHeight, pindexTip->nHeight, fInitialDownload, fReachedBestHeader);
}

void CReesistornodeSync::DoMaintenance(CConnman &connman)
{
