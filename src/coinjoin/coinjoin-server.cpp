...
void CCoinJoinServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        auto dmn = mnList.GetValidMNByCollateral(activeReesistornodeInfo.outpoint);
        if (!dmn) {
        ...
        if (vecSessionCollaterals.empty()) {
            {
                ...
                for (const auto& q : vecCoinJoinQueue) {
                    if (q.reesistornodeOutpoint == activeReesistornodeInfo.outpoint) {
...
//
// Check the mixing progress and send client updates if a Reesistornode
//
...
{
    if (!fReesistornodeMode) return; // check and relay final tx only on reesistornode
    ...
    // create and sign reesistornode dstx transaction
    if (!CCoinJoin::GetDSTX(hashTx)) {
        CCoinJoinBroadcastTx dstxNew(finalTransaction, activeReesistornodeInfo.outpoint, GetAdjustedTime());
        dstxNew.Sign();
...
//
// How does this work? Messages to Reesistornodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Reesistornode
...
/*
    ...
    allow endless transaction that would bloat Reesist and make it unusable. To
...
void CCoinJoinServer::CheckForCompleteQueue(CConnman& connman)
{
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CCoinJoinQueue dsq(nSessionDenom, activeReesistornodeInfo.outpoint, GetAdjustedTime(), true);
...
bool CCoinJoinServer::IsInputScriptSigValid(const CTxIn& txin)
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        // TODO we're using amount=0 here but we should use the correct amount. This works because Reesist ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
...
{
    if (!fReesistornodeMode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CCoinJoinQueue dsq(nSessionDenom, activeReesistornodeInfo.outpoint, GetAdjustedTime(), false);
...
{
    if (!fReesistornodeMode || nSessionID == 0 || IsSessionReady()) return false;
...
void CCoinJoinServer::SetState(PoolState nStateNew)
{
    ...
    if (nStateNew == POOL_STATE_ERROR) {
        LogPrint(BCLog::COINJOIN, "CCoinJoinServer::SetState -- Can't set state to ERROR as a Reesistornode. \n");
        return;
    }
...
{
    if (!fReesistornodeMode) return; // only run on reesistornodes
