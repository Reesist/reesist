...
static int64_t nLastReesistornodeList = 0;
...
void ClientModel::setReesistornodeList(const CDeterministicMNList& mnList)
{
    LOCK(cs_mnlinst);
    ...
    mnListCached = mnList;
    Q_EMIT reesistornodeListChanged();
}

CDeterministicMNList ClientModel::getReesistornodeList() const
{
    LOCK(cs_mnlinst);
    return mnListCached;
}

void ClientModel::refreshReesistornodeList()
{
    LOCK(cs_mnlinst);
    setReesistornodeList(m_node.evo().getListAtChainTip());
}
...
static void NotifyReesistornodeListChanged(ClientModel *clientmodel, const CDeterministicMNList& newList)
{
    int64_t now = 0;
    if (ninitialSync)
        now = GetTimeMillis();

    if (!ninitialSync || now - nLastReesistornodeList > MODEL_UPDATE_DELAY_SYNC) {
        clientmodel->setReesistornodeList(newList);
        nLastReesistornodeList = now;
    }
}
...
void ClientModel::subscribeToCoreSignals()
{
    ...
    m_handler_notify_reesistornodelist_changed = m_node.handleNotifyReesistornodeListChanged(boost::bind(NotifyReesistornodeListChanged, this, _1));
...
void ClientModel::unsubscribeFromCoreSignals()
{
    ...
    m_handler_notify_reesistornodelist_changed->disconnect();
