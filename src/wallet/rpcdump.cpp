...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"          (string, required) The Reesist address (or hex-encoded script)\n"
            "2. \"label\"            (string, optional, default=\"\") An optional label\n"
            "3. rescan               (boolean, optional, default=true) Rescan the wallet for transactions\n"
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Reesist address or script");
        }
    }
    if (fRescan)
    {
...
UniValue removeaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(address)) {
        if (!(IsMine(*pwallet, address) & ISMINE_WATCH_ONLY)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Reesist address - must be watch-only");
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The reesist address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to reesistd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Reesist Core %s\n", CLIENT_BUILD);
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("reesistcoreversion", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see reesistd log for details) and could "
