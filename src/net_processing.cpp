 ...
 *  Reesistornode outbound peers get quarter this delay. */
static const unsigned int INVENTORY_BROADCAST_INTERVAL = 5;
/** Maximum number of inventory items to send per transmission.
 *  Limits the impact of low-fee transaction floods.
 *  We have 4 times smaller block times in Reesist, so we need to push 4 times more invs per 1MB. */
...
void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, nProtocolVersion, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe,
            nonce, strSubVersion, nNodeStartingHeight, ::fRelayTxes, mnauthChallenge, pnode->m_reesistornode_connection));
...
std::chrono::microseconds CalculateObjectGetDataTime(const CInv& inv, std::chrono::microseconds current_time, bool use_inbound_delay) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (inv.type == MSG_TX && !fReesistornodeMode && use_inbound_delay) process_time += INBOUND_PEER_TX_DELAY;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_LEGACY_TXLOCK_REQUEST: // we treat legacy IX messages as TX messages
        {
            ...
            // reesistornode would not be able to exploit this to spam the network with specially
    ...
    /*
        Reesist Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (!vRecv.empty()) {
            bool fOtherReesistornode = false;
            vRecv >> fOtherReesistornode;
            if (pfrom->fInbound) {
                pfrom->m_reesistornode_connection = fOtherReesistornode;
                if (fOtherReesistornode) {
                    LogPrint(BCLog::NET_NETCONN, "peer=%d is an inbound reesistornode connection, not relaying anything to it\n", pfrom->GetId());
                    if (!fReesistornodeMode) {
                        LogPrint(BCLog::NET_NETCONN, "but we're not a reesistornode, disconnecting\n");
                        pfrom->fDisconnect = true;
                        return true;
                    }
                }
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (pfrom->nVersion >= LLMQS_PROTO_VERSION && !pfrom->m_reesistornode_probe_connection) {
        ...
        if (llmq::CLLMQUtils::IsWatchQuorumsEnabled() && pfrom->m_reesistornode_connection) {
    ...
    if (pfrom->nTimeFirstMessageReceived == 0) {
        ...
        if (pfrom->m_reesistornode_probe_connection && !pfrom->fFirstMessageIsMNAUTH) {
            LogPrint(BCLog::NET, "connection is a reesistornode probe but first received message is not MNAUTH, peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return false;
        }
    }
    ...
    if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::LEGACYTXLOCKREQUEST) {
        ...
        if (nInvType == MSG_DSTX) {
            ...
            for (int i = 0; i < 24 && pindex; ++i) {
                ...
                dmn = deterministicMNManager->GetListForBlock(pindex).GetMNByCollateral(dstx.reesistornodeOutpoint);
                if (dmn) break;
                pindex = pindex->pprev;
            }
            if(!dmn) {
                LogPrint(BCLog::COINJOIN, "DSTX -- Can't find reesistornode %s to verify %s\n", dstx.reesistornodeOutpoint.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!mmetaman.GetMetaInfo(dmn->proTxHash)->IsValidForMixingTxes()) {
                LogPrint(BCLog::COINJOIN, "DSTX -- Reesistornode %s is sending too many transactions %s\n", dstx.reesistornodeOutpoint.ToStringShort(), hashTx.ToString());
                return true;
            ...
            LogPrint(BCLog::COINJOIN, "DSTX -- Got Reesistornode transaction %s\n", hashTx.ToString());
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, ptx, &fMissingInputs /* pfMissingInputs */,
            ...
            if (nInvType == MSG_DSTX) {
                LogPrint(BCLog::COINJOIN, "DSTX -- Reesistornode transaction accepted, txid=%s, peer=%d\n",
    ...
    if (found)
    {
        ...
        reesistornodeSync.ProcessMessage(pfrom, strCommand, vRecv);
...
void PeerLogicValidation::EvictExtraOutboundPeers(int64_t time_in_seconds)
{
    ...
    if (extra_peers > 0) {
        ...
        connman->ForEachNode([&](CNode* pnode) {
            ...
            // Don't disconnect reesistornodes just because they were slow in block announcement
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            // Note: If this node is running in a Reesistornode mode, it makes no sense to delay outgoing txes
            // because we never produce any txes ourselves i.e. no privacy is lost in this case.
            bool fSendTrickle = pto->fWhitelisted || fReesistornodeMode;
            if (pto->nNextInvSend < current_time) {
                ...
                if (pto->fInbound) {
                    ...
                    // and quarter the delay for Reesistornode outbound peers, as there is even less privacy concern in this case.
