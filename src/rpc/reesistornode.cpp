...
UniValue reesistornodelist(const JSONRPCRequest& request);

void reesistornode_list_help()
{
    throw std::runtime_error(
            "reesistornodelist ( \"mode\" \"filter\" )\n"
            "Get a list of reesistornodes in different modes. This call is identical to 'reesistornode list' call.\n"
            "\nArguments:\n"
            "1. \"mode\"      (string, optional/required to use filter, defaults = json) The mode to run list in\n"
            "2. \"filter\"    (string, optional) Filter results. Partial match by outpoint by default in all modes,\n"
            "                                    additional matches in some modes are also available\n"
            "\nAvailable modes:\n"
            "  addr           - Print ip address associated with a reesistornode (can be additionally filtered, partial match)\n"
            ...
            "  owneraddress   - Print the reesistornode owner Reesist address\n"
            "  payee          - Print the reesistornode payout Reesist address (can be additionally filtered,\n"
            "                   partial match)\n"
            "  pubKeyOperator - Print the reesistornode operator public key\n"
            "  status         - Print reesistornode status: ENABLED / POSE_BANNED\n"
            "                   (can be additionally filtered, partial match)\n"
            "  votingaddress  - Print the reesistornode voting Reesist address\n"
        );
}

UniValue reesistornode_list(const JSONRPCRequest& request)
{
    if (request.fHelp)
        reesistornode_list_help();
    ...
    return reesistornodelist(newRequest);
}

void reesistornode_connect_help()
{
    throw std::runtime_error(
            "reesistornode connect \"address\"\n"
            "Connect to given reesistornode\n"
            "\nArguments:\n"
            "1. \"address\"      (string, required) The address of the reesistornode to connect\n"
        );
}

UniValue reesistornode_connect(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2)
        reesistornode_connect_help();

    std::string strAddress = request.params[1].get_str();

    CService addr;
    if (!Lookup(strAddress.c_str(), addr, 0, false))
        throw JSONRPCError(RPC_INTERNAL_ERROR, strprintf("Incorrect reesistornode address %s", strAddress));

    // TODO: Pass CConnman instance somehow and don't use global variable.
    g_connman->OpenReesistornodeConnection(CAddress(addr, NODE_NETWORK));
    if (!g_connman->IsConnected(CAddress(addr, NODE_NETWORK), CConnman::AllNodes))
        throw JSONRPCError(RPC_INTERNAL_ERROR, strprintf("Couldn't connect to reesistornode %s", strAddress));

    return "successfully connected";
}

void reesistornode_count_help()
{
    throw std::runtime_error(
            "reesistornode count\n"
            "Get information about number of reesistornodes.\n"
        );
}

UniValue reesistornode_count(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        reesistornode_count_help();
...
UniValue GetNextReesistornodeForPayment(int heightShift)
{
    auto mnList = deterministicMNManager->GetListAtChainTip();
...
void reesistornode_winner_help()
{
    if (!IsDeprecatedRPCEnabled("reesistornode_winner")) {
        throw std::runtime_error("DEPRECATED: set -deprecatedrpc=reesistornode_winner to enable it");
    }

    throw std::runtime_error(
            "reesistornode winner\n"
            "Print info on next reesistornode winner to vote for\n"
        );
}

UniValue reesistornode_winner(const JSONRPCRequest& request)
{
    if (request.fHelp || !IsDeprecatedRPCEnabled("reesistornode_winner"))
        reesistornode_winner_help();

    return GetNextReesistornodeForPayment(10);
}

void reesistornode_current_help()
{
    if (!IsDeprecatedRPCEnabled("reesistornode_current")) {
        throw std::runtime_error("DEPRECATED: set -deprecatedrpc=reesistornode_current to enable it");
    }

    throw std::runtime_error(
            "reesistornode current\n"
            "Print info on current reesistornode winner to be paid the next block (calculated locally)\n"
        );
}

UniValue reesistornode_current(const JSONRPCRequest& request)
{
    if (request.fHelp || !IsDeprecatedRPCEnabled("reesistornode_current"))
        reesistornode_current_help();

    return GetNextReesistornodeForPayment(1);
}

#ifdef ENABLE_WALLET
void reesistornode_outputs_help()
{
    throw std::runtime_error(
            "reesistornode outputs\n"
            "Print reesistornode compatible outputs\n"
        );
}

UniValue reesistornode_outputs(const JSONRPCRequest& request)
{
...
void reesistornode_status_help()
{
    throw std::runtime_error(
            "reesistornode status\n"
            "Print reesistornode status information\n"
        );
}

UniValue reesistornode_status(const JSONRPCRequest& request)
{
    if (request.fHelp)
        reesistornode_status_help();

    if (!fReesistornodeMode)
        throw JSONRPCError(RPC_INTERNAL_ERROR, "This is not a reesistornode");

    UniValue mnObj(UniValue::VOBJ);

    // keep compatibility with legacy status for now (might get deprecated/removed later)
    mnObj.pushKV("outpoint", activeReesistornodeInfo.outpoint.ToStringShort());
    mnObj.pushKV("service", activeReesistornodeInfo.service.ToString());

    auto dmn = deterministicMNManager->GetListAtChainTip().GetMN(activeReesistornodeInfo.proTxHash);
    if (dmn) {
    ...
    mnObj.pushKV("state", activeReesistornodeManager->GetStateString());
    mnObj.pushKV("status", activeReesistornodeManager->GetStatus());
...
void reesistornode_winners_help()
{
    throw std::runtime_error(
            "reesistornode winners ( count \"filter\" )\n"
            "Print list of reesistornode winners\n"
            "\nArguments:\n"
...
UniValue reesistornode_winners(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        reesistornode_winners_help();
...
void reesistornode_payments_help()
{
    throw std::runtime_error(
            "reesistornode payments ( \"blockhash\" count )\n"
            "\nReturns an array of deterministic reesistornodes and their payments for the specified block\n"
            "\nArguments:\n"
            ...
            "       \"amount\": n                   (numeric) Amount received in this block by all reesistornodes\n"
            "       \"reesistornodes\": [              (array) Reesistornodes that received payments in this block\n"
            "          {\n"
            "             \"proTxHash\": \"xxxx\",    (string) The hash of the corresponding ProRegTx\n"
            "             \"amount\": n             (numeric) Amount received by this reesistornode\n"
...
UniValue reesistornode_payments(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3) {
        reesistornode_payments_help();
    }
    ...
    while (vecPayments.size() < std::abs(nCount) != 0 && pindex != nullptr) {
        ...
        std::vector<CTxOut> voutReesistornodePayments, voutDummy;
        ...
        FillBlockPayments(dummyTx, pindex->nHeight, blockReward, voutReesistornodePayments, voutDummy, specialTxFees);

        UniValue blockObj(UniValue::VOBJ);
        CAmount payedPerBlock{0};

        UniValue reesistornodeArr(UniValue::VARR);
        UniValue protxObj(UniValue::VOBJ);
        UniValue payeesArr(UniValue::VARR);
        CAmount payedPerReesistornode{0};

        for (const auto& txout : voutReesistornodePayments) {
            UniValue obj(UniValue::VOBJ);
            CTxDestination dest;
            ExtractDestination(txout.scriptPubKey, dest);
            ...
            payedPerReesistornode += txout.nValue;
        ...
        protxObj.pushKV("amount", payedPerReesistornode);
        protxObj.pushKV("payees", payeesArr);
        payedPerBlock += payedPerReesistornode;
        reesistornodeArr.push_back(protxObj);
        ...
        blockObj.pushKV("reesistornodes", reesistornodeArr);
...
[[ noreturn ]] void reesistornode_help()
{
    throw std::runtime_error(
        "reesistornode \"command\" ...\n"
        "Set of commands to execute reesistornode related actions\n"
        "\nArguments:\n"
        "1. \"command\"        (string or set of strings, required) The command to execute\n"
        "\nAvailable commands:\n"
        "  count        - Get information about number of reesistornodes\n"
        "  current      - DEPRECATED Print info on current reesistornode winner to be paid the next block (calculated locally)\n"
#ifdef ENABLE_WALLET
        "  outputs      - Print reesistornode compatible outputs\n"
#endif // ENABLE_WALLET
        "  status       - Print reesistornode status information\n"
        "  list         - Print list of all known reesistornodes (see reesistornodelist for more info)\n"
        "  payments     - Return information about reesistornode payments in a mined block\n"
        "  winner       - DEPRECATED Print info on next reesistornode winner to vote for\n"
        "  winners      - Print list of reesistornode winners\n"
        );
}

UniValue reesistornode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "list") {
        return reesistornode_list(request);
    } else if (strCommand == "connect") {
        return reesistornode_connect(request);
    } else if (strCommand == "count") {
        return reesistornode_count(request);
    } else if (strCommand == "current") {
        return reesistornode_current(request);
    } else if (strCommand == "winner") {
        return reesistornode_winner(request);
#ifdef ENABLE_WALLET
    } else if (strCommand == "outputs") {
        return reesistornode_outputs(request);
#endif // ENABLE_WALLET
    } else if (strCommand == "status") {
        return reesistornode_status(request);
    } else if (strCommand == "payments") {
        return reesistornode_payments(request);
    } else if (strCommand == "winners") {
        return reesistornode_winners(request);
    } else {
        reesistornode_help();
    }
}
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "reesist",               "reesistornode",             &reesistornode,             {} },
    { "reesist",               "reesistornodelist",         &reesistornodelist,         {} },
};

void RegisterReesistornodeRPCCommands(CRPCTable &t)
{
