...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"reesist\" activates all Reesist-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "reesist")
            + HelpExampleRpc("debug", "reesist+net")
        );
...
UniValue mnsync(const JSONRPCRequest& request)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.pushKV("AssetID", reesistornodeSync.GetAssetID());
        objStatus.pushKV("AssetName", reesistornodeSync.GetAssetName());
        objStatus.pushKV("AssetStartTime", reesistornodeSync.GetAssetStartTime());
        objStatus.pushKV("Attempt", reesistornodeSync.GetAttempt());
        objStatus.pushKV("IsBlockchainSynced", reesistornodeSync.IsBlockchainSynced());
        objStatus.pushKV("IsSynced", reesistornodeSync.IsSynced());
        return objStatus;
    }

    if(strMode == "next")
    {
        reesistornodeSync.SwitchToNextAsset(*g_connman);
        return "sync updated to " + reesistornodeSync.GetAssetName();
    }

    if(strMode == "reset")
    {
        reesistornodeSync.Reset(true);
        return "success";
    }
    return "failure";
}
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given reesist address.\n"
            ...
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The reesist address to validate\n"
            "\nResult:\n"
            "{\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The reesist address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "reesist-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nreesist-cli has the option -getinfo to collect and format these in the old format."
    );
}
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Reesist features */
    { "reesist",               "mnsync",                 &mnsync,            {} },
    { "reesist",               "spork",                  &spork,             {"arg0","value"} },
