...
- Fixed memory issues
- Fixed remote reesistornode issues
...
- saction for Darksend. These are special transactions that require a signature that only the reesistornodes can create.
...
- Clients remember which reesistornodes they've connected to in the past and won't use them against.
...
- Split up darksend.cpp into reesistornode.cpp, activereesistornode.cpp and instantx.cpp
- Added modular ProcessMessages for Darksend, Reesistornodes and InstantX
- Client can now join sessions with any other users

0.10.15.20/21 Release notes
---------------------------

- added bloom filters for dsee/dseep broadcasts, moved expensive dsee search, reesistornode vote caching
...
- Fixed reesistornode payment check for out-of-order blocks
...
- reesistornode only take newest dsee now


0.10.15.15 Release notes
------------------------

- fixed fragmentation issue, allow reesistornodes to update pubkey2
...
- Fixed the stuck blockchain (due to reesistornode payments)
- Improves collateral creation for those getting "invalid collateral"
- Minimum confirmation requirements before reesistornode payment.
  Must have more confirmations then there are reesistornodes.
...
- Changes the reesistornode payments keys, you'll only recieve reesistornode
...
- I've made some huge progress with enforcement. Each reesistornode will
...
- Fixed some issues with reesistornode payments syncing
...
- Fixed some issues with the way the reesistornode payment list
  was being handled
- Reesistornode confirmation error now displays 200 required instead of 6
...
- security update: This update simply verifies that the signature was
  not forged and that the reesistornodes entering the list are authentic.
