...
*Setup instructions for a Gitian build of Reesist Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Reesist
...
- [Building Reesist Core](#building-reesist-core)
...
Clone the git repositories for Reesist Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/reesist/reesist
...
in the Reesist Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Reesist Core
----------------

To build Reesist Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Reesist Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/reesist/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/reesist/reesist
...
For example:
```bash
URL=https://github.com/crowning-/reesist.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit reesist=${COMMIT} --url reesist=${URL} ../reesist/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit reesist=${COMMIT} --url reesist=${URL} ../reesist/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit reesist=${COMMIT} --url reesist=${URL} ../reesist/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the reesist git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../reesist/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/reesist
SIGPATH=/some/root/path/reesist-detached-sigs

./bin/gbuild --url reesist=${BTCPATH},signature=${SIGPATH} ../reesist/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/reesist-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/reesist-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/reesist-osx-build.assert
```
...
[reesist/gitian.sigs](https://github.com/reesist/gitian.sigs/) repository, or if that's not possible to create a pull
request.
