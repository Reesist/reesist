...
- [Development guidelines](#development-guidelines)
    - [General Reesist Core](#general-reesist-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Reesist Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Raptor3um/reesist/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/reesistd -printtoconsole
```
...
Reesist Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadOpenReesistornodeConnections : Opens network connections to reesistornodes.
...
However, in open source software such as Reesist Core, where everyone uses
...
pay attention to for reviewers of Reesist Core code.

General Reesist Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Reesist Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Reesist Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `reesist-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `reesist-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Reesist Core, the wallet was always
