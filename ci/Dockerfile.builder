...
# reesist_hash
RUN git clone https://github.com/reesist/reesist_hash
RUN cd reesist_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} reesist
RUN useradd -u ${USER_ID} -g reesist -s /bin/bash -m -d /reesist reesist
...
RUN mkdir /reesist-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /reesist-src && \
